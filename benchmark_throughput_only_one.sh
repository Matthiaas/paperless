...
#BSUB -P paperless
#BSUB -J throughput
#BSUB -R "rusage[scratch=2120,mem=5120] span[hosts=1] select[model==EPYC_7742]"
    ...
    for i in "${RANKS[@]}"; do
        for j in "${VALLEN[@]}"; do
          for UPDATE_RATIO in "${UPDATE_RATIOS[@]}"; do
             echo mpirun --report-bindings --map-by node:PE=$c -np $i ./build/throughput_$KV $KEYLEN $j $COUNT $UPDATE_RATIO $DATA_LOCATION/$KV SEQ 1000
             mpirun --report-bindings --map-by node:PE=$c -np $i ./build/throughput_$KV $KEYLEN $j $COUNT $UPDATE_RATIO $DATA_LOCATION/$KV SEQ 1000
           done
        done
        echo ""
    done
