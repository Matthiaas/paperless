...
sed  's/report_one_host/report_n_host/g' benchmark_op_times.sh | bsub -R "rusage[scratch=5120,mem=5120]  span[ptile=2] select[model==EPYC_7742]"
sed  's/report_one_host/report_n_host/g' benchmark_seq_vs_rel.sh | bsub -R "rusage[scratch=5120] span[ptile=2] select[model==EPYC_7742]"
sed  's/report_one_host/report_n_host/g' benchmark_throughput.sh |  bsub -R "rusage[scratch=5120,mem=5120] span[ptile=2] select[model==EPYC_7742]"
