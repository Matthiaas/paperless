...
class Element {
 public:
  ...
  [[nodiscard]] const ElementView &GetView() const { return el_; }

  [[nodiscard]] char *Value() const { return el_.value_; }

  [[nodiscard]] size_t Length() const { return el_.len_; }
...
class Tomblement {
 public:
  ...
  [[nodiscard]] static Tomblement createFromBuffWithoutCopy(char *v,
                                                            size_t len) {
    Tomblement res;
    res.value_ = v;
    res.len_ = len - 1;
  ...
  [[nodiscard]] Element CopyToElement() const { return Element(Value(), len_); }
  ...
  [[nodiscard]] char *Value() const { return value_ + 1; }

  [[nodiscard]] size_t Length() const { return len_; }

  [[nodiscard]] bool Tombstone() const { return value_[0]; }

  [[nodiscard]] char *GetBuffer() const { return value_; }

  [[nodiscard]] size_t GetBufferLen() const { return len_ + 1; }
  ...
  bool operator<(const Tomblement &other) const {
    ...
    for (size_t i = 0; i < len_ + 1; i++) {
      if (value_[i] != other.value_[i]) return value_[i] < other.value_[i];
    }
    return false;
  }
